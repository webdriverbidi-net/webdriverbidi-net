// <copyright file="FirefoxNightlyLocator.cs" company="WebDriverBiDi.NET Committers">
...
/// <summary>
/// Locator for Firefox binaries on the Nightly channel.
/// </summary>
public class FirefoxNightlyLocator : FirefoxLocator
{
    private static readonly string FirefoxNightlyCacheDir = Path.Combine(
        BaseCacheDir,
        "firefox-nightly");
    ...
    /// </summary>
    protected override string BrowserName => "Firefox Nightly";

    /// <summary>
    /// Gets the cache directory specific to this browser, which is a subdirectory of the base cache directory.
    /// </summary>
    protected override string CacheDir => FirefoxNightlyCacheDir;
    ...
    protected override string GetInstallerFileName()
    {
        ...
        {
            return "firefox-nightly.dmg";
        }

        if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
        {
            return "firefox-nightly.tar.xz";
        }

        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            return "firefox-nightly-installer.exe";
        }
    ...
    protected override Task<string> GetDownloadUrl()
    {
        if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
        {
            return Task.FromResult("https://download.mozilla.org/?product=firefox-nightly-latest-ssl&os=osx&lang=en-US");
        }

        if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
        {
            return Task.FromResult("https://download.mozilla.org/?product=firefox-nightly-latest-ssl&os=linux64&lang=en-US");
        }

        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            return Task.FromResult("https://download.mozilla.org/?product=firefox-nightly-latest-ssl&os=win64&lang=en-US");
        }
    ...
    protected override string GetExpectedExecutablePath()
    {
        if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
        {
            return Path.Combine(this.CacheDir, "Firefox Nightly.app", "Contents", "MacOS", "firefox");
        }
