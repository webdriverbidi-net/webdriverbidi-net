...
/// </summary>
public class BrowserGroup : IAsyncDisposable
{
    ...
    private BrowserGroup(BrowserLauncher launcher, BiDiDriver driver, ElementLocatorSettings locatorSettings)
    {
    ...
    /// <see cref="BrowserGroup"/> instance representing the running browser.
    /// </summary>
    ...
    /// <returns>A <see cref="Task"/> representing the asynchronous operation, containing the launched <see cref="BrowserGroup"/> instance.</returns>
    public static Task<BrowserGroup> LaunchAsync(BrowserKind browserKind, bool headless = false, ElementLocatorSettings? locatorSettings = null)
    {
        BrowserLauncherBuilder builder = new(browserKind);
        builder.WithHeadlessOption(headless);
        if (browserKind == BrowserKind.Safari)
        {
            // Safari cannot be cached in a non-default location.
            builder.AtDefaultInstallationLocation();
        }

        return LaunchAsync(builder, locatorSettings);
    }

    /// <summary>
    /// Launches a browser using the specified <see cref="BrowserLauncherBuilder"/> configuration,
    /// initializes a WebDriver BiDi session, and returns a <see cref="BrowserGroup"/> instance
    /// representing the running browser.
    /// </summary>
    /// <param name="launcherBuilder">The configured launcher builder to use.</param>
    /// <param name="locatorSettings">Optional <see cref="ElementLocatorSettings"/> to apply to element locators. If null, default settings are used.</param>
    /// <returns>A <see cref="Task"/> representing the asynchronous operation, containing the launched <see cref="BrowserGroup"/> instance.</returns>
    public static async Task<BrowserGroup> LaunchAsync(BrowserLauncherBuilder launcherBuilder, ElementLocatorSettings? locatorSettings = null)
    {
    ...
    private static async Task<BrowserGroup> LaunchAsync(BrowserLauncher launcher, ElementLocatorSettings? locatorSettings)
    {
        ...
        try
        {
            ...
            BrowserGroup repository = new(launcher, driver, settings);
