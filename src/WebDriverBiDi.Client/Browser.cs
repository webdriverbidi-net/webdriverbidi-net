...
public class Browser : IAsyncDisposable
{
    ...
    private readonly BrowserRepository repository;
    ...
    /// <param name="repository">The <see cref="BrowserRepository"/> that owns this browser.</param>
    /// <param name="locatorSettings">The <see cref="ElementLocatorSettings"/> to apply to element locators.</param>
    internal Browser(BiDiDriver driver, string userContextId, BrowserRepository repository, ElementLocatorSettings locatorSettings)
    {
        this.driver = driver;
    ...
    /// <summary>
    /// Gets the ID of the user context this browser represents.
    /// </summary>
    ...
    /// it from the owning <see cref="BrowserRepository"/>.
    /// </summary>
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        try
        {
            await this.driver.Browser.RemoveUserContextAsync(
                new WebDriverBiDi.Browser.RemoveUserContextCommandParameters(this.UserContextId))
    ...
    private void OnContextCreated(BrowsingContextEventArgs args)
    {
        ...
        if (args.UserContextId != this.UserContextId)
        {
            return;
        }

        if (this.pages.Any(p => p.BrowsingContextId == args.BrowsingContextId))
        {
            return;
        }
    ...
    {
        Page? page = this.pages.FirstOrDefault(p => p.BrowsingContextId == args.BrowsingContextId);
        if (page is not null)
        {
