...
public class BiDiDriver015CodeFixProviderTests
{
    ...
    /// <summary>
    /// Tests that the code fix replaces a string literal in the single-event constructor with the EventName property.
    /// </summary>
    /// <returns>A <see cref="Task"/> representing the asynchronous unit test.</returns>
    [Fact]
    public async Task CodeFix_ReplacesStringLiteralInSingleEventConstructor()
    {
        string testCode = """
            using System.Threading.Tasks;
            using WebDriverBiDi;
