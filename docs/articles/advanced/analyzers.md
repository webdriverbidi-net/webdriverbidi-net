...
|----|----------|----------------|
| **BIDI001** | Error | `RegisterModule()` called after `StartAsync()`. A `StopAsync()` call returns the driver to the not-started state, so registration after a stop is not reported. The started state is tracked through `if`/`else`, `switch` and `try`/`catch`/`finally`: a registration is reported only when the driver is started on every path that reaches it, so one in a `catch` after a `StartAsync()` in the `try` (which may have failed and rolled the driver back) or in a `switch` section other than the one that started the driver is not reported. A driver declared by a classic `await using (BiDiDriver driver = ...)` statement is tracked like a local |
...
| **BIDI006** | Warning | `EventObserver` not disposed or unobserved. A `using` declaration, a classic `using (observer) { ... }` statement, `observer.Dispose()`, `observer?.Dispose()`, `DisposeAsync()`, `Unobserve()`, `RemoveObserver(observer.Id)`, and returning or storing the observer all count as handling it |
