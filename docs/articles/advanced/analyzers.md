...
The rest of the library's asynchronous surface is reported on the same footing: a `Transport`'s or
`Connection`'s `ConnectAsync`, `DisconnectAsync`, `StartAsync`, `StopAsync`, `SendCommandAsync`,
`SendDataAsync` and `DisposeAsync`, an `EventObserver<T>`'s `WaitForCapturedTasksAsync`,
`WaitForCapturedTasksCompleteAsync` and `DisposeAsync`, and an `EventDataCollector<T>`'s
`DisposeAsync`. Discarding one of these leaves an observer subscribed, a collector's channel open, or
a connection and its receive loop running, and a wait whose task is discarded returns nothing the
caller can act on. What decides is the type that *declares* the operation: an override of one of them
