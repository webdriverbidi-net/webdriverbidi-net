...
Every intra-procedural rule takes its analysis roots from the statements of a method, a constructor, or a top-level program. The **state-tracking** rules — BIDI001, BIDI002, BIDI003, BIDI009, BIDI020, BIDI021, BIDI024, BIDI029 and BIDI030 — walk those statements in order and do not descend into a lambda, an anonymous method, or a local function, because that code runs when the delegate is invoked rather than where it is written. A driver or an observer *declared inside* one of those is therefore never tracked by them, and they report no diagnostic for it:
