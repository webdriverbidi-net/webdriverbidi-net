...
The analyzer package is marked as a development dependency, so `dotnet add package` writes `PrivateAssets="all"` for you and the analyzer does not flow to consumers of your own package. A hand-written `<PackageReference>` gets neither that attribute nor the assets filter, so write both when pinning an exact version (see [Pinning an exact version](../getting-started.md#pinning-an-exact-version) for why to pin at all):

```xml
<PackageReference Include="WebDriverBiDi.Analyzers" Version="0.0.61">
...
| **BIDI004** | Info | A module command, or `driver.ExecuteCommandAsync()`, that declares a `CancellationToken` parameter is called without one; suggests passing it. Every command of every module counts, including a command of your own module, so long as the method or one of its overloads takes a token. The driver's own lifecycle members are not commands and are left alone, and the long-running operations BIDI013 reports (`NavigateAsync`, `PrintAsync`, `ReloadAsync`, `StartAsync`, `WaitForCapturedTasksAsync`, `WaitForCapturedTasksCompleteAsync`) are reported there at Warning instead, never by both |
| **BIDI005** | Warning | A local subscription to an event — `AddObserver`, `AddDataCollector`, or `Subscribe` on a `ToObservable()` sequence, each of which needs the same remote subscription — whose event name is not included in a subscription sent from the same method, whether as `Session.SubscribeAsync(...)` or as the driver's `ExecuteCommandAsync(...)` given a `SubscribeCommandParameters` (the two send the same command). Both the list constructor (`new SubscribeCommandParameters(["a", "b"])`) and the single-event constructor (`new SubscribeCommandParameters(driver.Log.OnEntryAdded.EventName)`) are recognized; string literals, constants and `EventName` property accesses all resolve. When any part of the subscription cannot be read at the call site — the parameters object held in a variable, an events list held in a variable, a spread element such as `[..events]`, a non-constant array element, or an object initializer that adds to `Events` — the subscribed set is unknowable and no warning is reported, because reporting from an incomplete set would accuse code that does subscribe. Only a driver the method creates and keeps to itself is judged: a local initialized with `new`, or a `new` driver in the event access itself, that is not passed to other code. A driver received as a parameter, held in a field, returned by a call, or passed to a helper may be subscribed in code the method cannot see, such as a test fixture's set-up method, so no warning is reported for it. The same holds for a driver whose session module is handed on, as in `await SubscribeAsync(driver.Session)`, directly or through a `SessionModule` local that is itself passed, returned or stored, because the code it reaches can subscribe through it; handing on any other module does not count. Expression-bodied members are analyzed like block-bodied ones |
...
| **BIDI010** | Error | Async module command, `driver.ExecuteCommandAsync()`, one of the driver's own lifecycle operations (`StartAsync`, `StopAsync`, `RegisterTypeInfoResolverAsync`, `DisposeAsync`), or any asynchronous operation the library declares on a `Transport`, a `Connection`, an `EventObserver<T>` or an `EventDataCollector<T>` (connect, disconnect, send, the transport's own type-info registration, the waits for captured tasks, and disposal) not awaited (fire-and-forget), including one made through a null-conditional receiver (`driver?.Session.StatusAsync();`). A module is any type deriving from `Module`, whatever it is named. On a type derived from one of those four, an override of a library operation is judged and the deriving type's own asynchronous methods are not |
...
| **BIDI015** | Warning | String literal used for event name instead of `ObservableEvent.EventName`, in either `SubscribeCommandParameters` constructor form. The receiver is resolved from the expression the call roots in, which must be a bare identifier — a local, a parameter, or a field or property of the enclosing type written without `this.` — or a `this.`-qualified member; a driver reached through another object, as in `fixture.Driver.Session.SubscribeAsync(...)`, is not matched and nothing is reported. Expression-bodied members are analyzed like block-bodied ones |
| **BIDI016** | Warning | Deadlock-prone synchronization in an `async` event handler: `lock`, `Monitor.Enter`/`TryEnter`, `SemaphoreSlim.Wait`, `ManualResetEventSlim.Wait`, `CountdownEvent.Wait`, `WaitHandle.WaitOne` (which covers `Mutex`, `Semaphore`, `ManualResetEvent` and `AutoResetEvent`), `SynchronizationContext.Send`, `Task.WaitAll`/`WaitAny`. A wait given an explicit timeout of zero returns at once and is not reported, as for BIDI007, and `Monitor.TryEnter(gate)` — the overload with no timeout, which takes the lock or returns at once — is not reported at all. Blocking calls such as `.Result` and `.Wait()` are BIDI007. `RunHandlerAsynchronously` suppresses the diagnostic only for what runs after the handler's first `await`: the library runs a `Func<T, Task>` handler on the dispatching thread up to that point whatever the option says, so a primitive before it is still reported, with a message saying so. An `Action<T>` handler is queued whole, so nothing in it is reported |
...
| **BIDI020** | Error | `WaitForCapturedTasksAsync()` or `WaitForCapturedTasksCompleteAsync()` called without a prior `StartCapturingTasks()` in the same method. The session is tracked per local observer through `if`/`else`, `switch`, `try`/`catch`/`finally` and `for`/`foreach`/`while` loops, and a wait is reported only when no path that reaches it can have opened a session, so a `StopCapturingTasks()` in a `catch` that rethrows does not condemn the wait after the `try`. Assigning to the variable replaces the tracked observer: one assigned a fresh `AddObserver` result has no session, so a wait on it before a start is reported, while one assigned from anywhere else (a method that may have started a session, or a `??=` that may not rebind at all) is treated as possibly capturing and is not |
...
| **BIDI030** | Warning | `StartCapturingTasks()` called on an `EventObserver` whose capture session is already active, which throws `WebDriverBiDiException`. The companion of BIDI020. A `StopCapturingTasks()` ends the session; so may a `WaitForCapturedTasksAsync` or `WaitForCapturedTasksCompleteAsync`, which ends it when it collects its full batch, so a start after either is never reported. The session is tracked through `if`/`else`, `switch`, `try`/`catch`/`finally` and `for`/`foreach`/`while` loops, and a start is reported only when a session is active on every path that reaches it. Assigning to the variable replaces the tracked observer, whose session is unknown, so a start after the assignment is not reported |
...
| **BIDI039** | Error | `GetCollectedEventData()` is called on an `EventDataCollector<T>` after the collector has been disposed, by `Dispose()`, `DisposeAsync()` or the implicit disposal of an `await using`, and throws `ObjectDisposedException`. The companion of BIDI038 for the handle `AddDataCollector` returns. `ToString()` and a second disposal are not reported, because neither throws on a disposed collector, and neither is the `Events` property, which is not a call and whose sequence ends rather than throwing. Tracked per local variable through the same branches, loops and handlers as BIDI029, and reported only when the collector is disposed on every path that reaches the use |
...
- **BIDI001** — Moves the `RegisterModule()` statement before `StartAsync()`, together with any local declarations it depends on that were declared after the start (`CustomModule module = new(driver);`). No fix is offered when such a declaration awaits something (it may need the started driver), when a statement between the start and the registration assigns a variable the moved statements read (an assignment, or an `out` or `ref` argument), since it could not move with them, or when the registration is not a statement of the same block as the `StartAsync()` — inside an `if`, a loop or a `try`, braced or not — since hoisting it out would change whether or how often it runs. No fix is offered where the registration is not inside a method with a block body — a constructor, an expression-bodied member, or a top-level program — though the diagnostic is still reported there
- **BIDI002** — Moves the `RegisterEvent()` statement before `StartAsync()`, with the same dependency handling as BIDI001 and the same conditions for offering no fix
- **BIDI003** — Moves the `RegisterTypeInfoResolverAsync()` statement before `StartAsync()`, with the same dependency handling as BIDI001 and the same conditions for offering no fix
...
- **BIDI009** — Moves the command execution after the existing `StartAsync` call on the same driver. The command may move into a block nested around the start that is entered unconditionally and exactly once, such as a `try` or a `using` body, but not into an `if`, a loop or a `switch` section, and not into an unbraced embedded statement, which is no statement list at all. A command that declares a local is moved only when every use of that local still follows the declaration and stays inside the block it lands in. As for BIDI001, no fix is offered where the command is not inside a method with a block body — a constructor, an expression-bodied member, or a top-level program — though the diagnostic is still reported there
- **BIDI010** — Adds `await` to the fire-and-forget command, offered only where `await` is legal: the nearest enclosing function must be `async`, or the call must be a top-level statement. A call made through a null-conditional receiver is awaited as a whole rather than at the inner invocation, and is coalesced to an already-completed awaitable — `await (driver?.Session.StatusAsync() ?? Task.CompletedTask);`, or `?? default` where the operation returns a `ValueTask` — so that a null receiver still does nothing, as `?.` asked, instead of throwing on an awaited null
...
- **BIDI023** — Same fix as BIDI007: adds `ObservableEventHandlerOptions.RunHandlerAsynchronously` to the `AddObserver` call, converting a non-`async` `Task`-returning lambda to an `async` one first; no fix for method-group handlers, nor for a handler held in a local rather than written in the call
...
**Info.** A command that accepts a `CancellationToken` is called without one. See [API Design Guide](api-design.md).

Every method of a module is judged, not a fixed list of names: if the command you are calling declares a token parameter, or has an overload that does, and you pass none, the rule suggests one. That covers the commands of your own `Module` subclasses as well, whatever their token parameter is named — the code fix names the argument after the parameter it found. Three kinds of call are left alone: the long-running operations BIDI013 already reports at `Warning`, so that one omission is never reported twice; the driver's own lifecycle members, which are not commands; and the invocation of a delegate, which resolves to `Invoke` rather than to a command.

### BIDI005

**Warning.** An event is subscribed to locally but its name is not included in `Session.SubscribeAsync()`, so the remote end never sends it and nothing arrives. All three subscription shapes count — `AddObserver`, `AddDataCollector` and `Subscribe` on a `ToObservable()` sequence — and the code fix adds the event to an existing subscription in the same method. See [Common Pitfalls — Event Subscription](../common-pitfalls.md#event-subscription).
...
The filter passed to `AddDataCollector` is judged as well. The collector registers it through an observer of its own with `RunHandlerSynchronously`, so it runs on the dispatching thread for every event, and it takes no handler options — there is nothing for the code fix to add, so none is offered and the message says to keep the filter to a test of the event data:

<!-- inline-csharp: written to trip the analyzer under discussion, so it cannot compile in the snippets project -->
```csharp
// Flagged: every event the collector sees waits on this.
var collector = driver.Log.OnEntryAdded.AddDataCollector(entry =>
{
    Thread.Sleep(100);
    return entry.Level == LogLevel.Error;
});

// Not flagged: the filter only tests the event data.
var errors = driver.Log.OnEntryAdded.AddDataCollector(entry => entry.Level == LogLevel.Error);
```
...
The rest of the library's asynchronous surface is reported on the same footing: a `Transport`'s
or `Connection`'s `ConnectAsync`, `DisconnectAsync`, `StartAsync`, `StopAsync`,
`SendCommandAsync`, `SendDataAsync` and `DisposeAsync`, a `Transport`'s own
`RegisterTypeInfoResolverAsync`, an `EventObserver<T>`'s `WaitForCapturedTasksAsync`,
`WaitForCapturedTasksCompleteAsync` and `DisposeAsync`, and an `EventDataCollector<T>`'s
`DisposeAsync`. Discarding one of these leaves an observer subscribed, a collector's channel
open, or a connection and its receive loop running, and a wait whose task is discarded returns
nothing the caller can act on. What decides is the type that *declares* the operation: an override of one of them
on your own `Connection` subclass is judged, while an asynchronous method that subclass adds of its
own is not.
...
**Warning.** A value is written into one of the library's `[JsonExtensionData]` dictionaries — `AdditionalData`, `CapabilityRequest.AdditionalCapabilities` or `Command.AdditionalCommandProperties` — which is not AOT/trimming safe unless every value's type is registered. The attribute is what the rule matches on, so a dictionary added to the library later is covered without changing the rule. See [API Design Guide — Protocol Extensions via AdditionalData](api-design.md#protocol-extensions-via-additionaldata) and [AOT Compatibility](aot-compatibility.md).
...
The filter passed to `AddDataCollector` is judged as well: it runs on the dispatching thread for every event and takes no handler options, so a module command issued from it can neither be offloaded nor made safe by an option, and no code fix is offered. Collect the event and issue the command where the collected data is read.
...
As with BIDI020, the state is tracked per local variable and merged across branches and `for`/`foreach`/`while` loops; a start is reported only when a session is certainly active on every path that reaches it. Assigning to the variable replaces the observer it names, and the replacement's session is unknown, so a start after the assignment is not reported either.
...
**Error.** `GetCollectedEventData()` is called on an `EventDataCollector<T>` after the collector has been disposed. Disposal unsubscribes the collector's observer from the event and completes the channel it collects into, and reading the collected data throws `ObjectDisposedException` from that point on. A collector cannot be resubscribed, so the remedy is a new collector from the event. This is the companion of BIDI038 for the handle `AddDataCollector` returns.
...
// Flagged: the collector is disposed.
entries = collector.GetCollectedEventData();

// Not flagged: neither throws on a disposed collector.
string description = collector.ToString();
collector.Dispose();
```

`GetCollectedEventData()` is the collector's only member that throws once it is disposed, so it is the only one reported. The `Events` sequence is not: its channel is completed by the disposal, so an `await foreach` over it simply ends, which the rule cannot express as a call in any case.

The rule shares BIDI029's walk, so it judges a local collector the same way: the disposal must be certain on every path that reaches the use, a collector disposed or rebound inside a lambda or local function is not tracked at all, and only a call on the collector variable itself is judged. Written as the documentation recommends — `await using EventDataCollector<T> collector = ...` — disposal happens at the end of the scope, so nothing inside it can be reported.
...
| Use after disposal (BIDI029, BIDI038, BIDI039) | [Core Concepts - Proper Disposal](../core-concepts.md#proper-disposal) |
...
|-------|------------------------|-------|
| **Intra-procedural** — single method body | The analyzer walks one method at a time and correlates statements within that method (e.g., "was `StartAsync` called before this line?"). It cannot see into other methods. | BIDI001, BIDI002, BIDI003, BIDI005, BIDI006, BIDI009, BIDI012, BIDI014, BIDI015, BIDI020, BIDI021, BIDI024, BIDI029, BIDI030, BIDI032, BIDI036, BIDI038, BIDI039 |
...
| **Per-invocation with handler-body descent** — call site plus the callback it passes | The analyzer inspects each matching `AddObserver(...)` call — and, for BIDI007 and BIDI023, each `AddDataCollector(...)` call — and also walks into the callback body to look for patterns. When the handler is an inline lambda, the body is right there. When the handler is passed as a method reference (e.g., `AddObserver(this.HandleEvent)`), BIDI007 and BIDI023 resolve the reference and walk that method body too, in whichever file it is declared (for a partial method, its implementing declaration), and they walk a handler held in a local initialized with a lambda as that lambda; BIDI016 inspects only inline `async` lambda handlers and does not follow method references. None of them continue transitively into further methods that the handler body calls. | BIDI007, BIDI016, BIDI023 |
...
An event can be subscribed to in three ways — `AddObserver`, `AddDataCollector`, and `Subscribe` on a `ToObservable()` sequence — and the rules differ in how many of them they judge, because the three deliver events differently. BIDI005, BIDI006, BIDI031 and BIDI032 judge all three: every one of them needs the same remote subscription and holds an undisposed handle. BIDI007 and BIDI023 add the filter of `AddDataCollector`, which the library runs synchronously on the dispatching thread, but not `Subscribe`, whose `IObserver<T>.OnNext` is called from a background drain loop and blocks nothing. BIDI016 and BIDI025 stay with `AddObserver` alone: both judge an `async` handler, and a filter is a `Func<T, bool>` that no `async` lambda can bind to.
...
Every intra-procedural rule takes its analysis roots from the statements of a method, a constructor, or a top-level program. The **state-tracking** rules — BIDI001, BIDI002, BIDI003, BIDI009, BIDI020, BIDI024, BIDI029, BIDI030, BIDI038 and BIDI039 — walk those statements in order and do not descend into a lambda, an anonymous method, or a local function, because that code runs when the delegate is invoked rather than where it is written. A driver or an observer *declared inside* one of those is therefore never tracked by them, and they report no diagnostic for it:
...
- **Using a driver after `DisposeAsync()` (BIDI029), an observer's capture members after disposing it (BIDI038), or a data collector's `GetCollectedEventData()` after disposing it (BIDI039):** throws `ObjectDisposedException`.
