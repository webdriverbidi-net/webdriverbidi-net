...
The failure surfaces the same way whichever event the observer was added to — `driver.OnLogMessage`, `transport.OnLogMessage`, or `connection.OnLogMessage` directly: the observer's exception is reported, identifying that observer, and routed through [`EventHandlerExceptionBehavior`](error-handling.md#transport-error-behavior-configuration), so the interrupted operation still completes. No throwing observer of any event fails the operation that raised the event. That holds for a connection used on its own, without a transport, too: having no error pipeline to route the failure to, it records it as the `EventHandlerError` EventSource event (see [Observability](observability.md)).
